...
def c3_from_bin_fun(s1, s2, d, binfileA, binfileB, day):
    """
    ...
    for ia, asta in enumerate(sources):
        ...
        for ib, bsta in enumerate(receivers):#[ia+1:]):
            ...
            for ic, csta in enumerate(stations):
                ...
                print '>> We look for ', pair, ' with ', csta.station, '-%s- '%day, '| ',ic 
                # some can be empty
...
def prestackc3(argv):
    ...
    shortWin = int(p['shortWin']*df)  # short win if want a moving window. If shortwin=coda_len -> no moving window
    dir_prestack = '../c1/bins/corr/no_rotation/'
    sorted = True#False

    try:
        opts, args = getopt.getopt(argv,"ha:b:d:v",["help", "staTarget1=", "staTarget2=", "day=", "verbose"])
    except getopt.GetoptError,  e:
        print e
        print "binc3.py -a <staTarget1> -b <staTarget2> -d <day> -v <verbose> -h <help>"
        sys.exit(2)
    for opt, arg in opts:
        if opt in ('-h', '--help'):
            print "binc2.py -a <staTarget1> -b <staTarget2> -d <day> -v <verbose> -h <help>"
            sys.exit(2)
    ...
    binfileA = glob.glob(dir_prestack+s1+'/*%03d*EE*'%day)
    binfileB = glob.glob(dir_prestack+s2+'/*%03d*EE*'%day)
    print binfileA, binfileB
    if binfileA and binfileB !=[]:
        pass
    else:
        print 'One file empty >> going out.'
        sys.exit(2)
...
#            if ia >50:continue
    ...
    #        binfileA = [a]
    #        binfileB = gtmp
    c3 = c3_from_bin_fun(s1, s2, d, binfileA, binfileB, day)
 ...
 #  #         if ia >50:continue
    ...
    try: os.makedirs('pc3/EE/%s/%s/'%(s1,s2))
    except: pass
    t.write('pc3/EE/%s/%s/pc3.%s.%s.%03d.EE.mseed'%(s1, s2, s1, s2, day), format='MSEED')
